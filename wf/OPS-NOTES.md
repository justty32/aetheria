...
- 🔧 **限流的歸屬判斷走「路徑」，不走名稱、也不走行程樹**（前兩種昨天都試過都錯）。
  ...
  **行程 cwd 仍是啟動它的地方**（我從 repo 根派工，所以自己的 codex 一個都認不出來）。
  ...
  辨識特徵：**我的是 `codex exec -C ...`；互動式（如 `codex --model ...`）或桌面版的都不是我的**。
  腳本在 `scratchpad/watchdog-v3.sh`（**寫成檔案不要內嵌 Monitor**，內嵌就改不動）。
- ⚠ **`uptime` 的 load average 判斷不了核心忙不忙**：codex 大半時間在等 API。
  2026-08-22 load 僅 0.12，但機器上同時有 8 個 codex（多路 agent 並行）。**看行程清單，不要看 load。**
...
- 🔧 **主 repo 冷啟動 configure 會失敗**（2026-08-27 波管家實測，凍結期後第一件會撞的事）：
  cache 指向真 Godot 4.7.2 但專案期望 4.7.1，且 vcpkg 已搬家（舊 `/home/lorkhan/vcpkg`）。
  可用配方：`-DAETHERIA_GODOT_BIN=/bin/false
  -DCMAKE_TOOLCHAIN_FILE=/home/lorkhan/dev/vcpkg/scripts/buildsystems/vcpkg.cmake`。
