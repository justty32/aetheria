...
**實際的收件匣（放信處）在 `wf/` 根的 [`inbox/`](../../inbox/)**——本工作流管「怎麼用它」，那個資料夾只放信、保持乾淨。
...
> 本專案目前尚在規劃階段、單一 session 開發，**尚無已知通信對象**。未來若有其他 agent（例如同 `~/repo/game_dev/` 或 `~/repo/moddings/` 下的協作 agent）需要交接，同下方方式寄收信即可；已知對象記到 [CONTACTS.md](CONTACTS.md)。

## 地址 = inbox 資料夾位置

**用「在哪個資料夾工作」來認 agent。** 一個 agent 的地址，就是它工作資料夾底下的 `inbox/`（本專案的是 `wf/` 根的 [`inbox/`](../../inbox/)）。
...
> 放信處是 `wf/` 根的 [`inbox/`](../../inbox/)（含 `done/` 歸檔），**不在本資料夾**——本資料夾只放使用方式。本入口檔若膨脹，照 [DEV-GUIDE「結構整理原則」](../../DEV-GUIDE.md) 拆。
