...
新增 / 修改 aetheria 功能的工作流。這是本工作流的**入口**：先讀本檔，再往下深入。always-on 鐵律見 [AGENTS.md](../../../AGENTS.md)；要整理結構時參考 [DEV-GUIDE](../../DEV-GUIDE.md)（被動）；**程式碼慣例**見 [common/conventions](../common/conventions.md)。

## 流程

```
修改（增量）
  → 跑自動驗證（尚未建立，見 design/；日後補上 build / test / lint 指令）綠燈
