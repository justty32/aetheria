...
| `core/runtime/` | 跨 zone API；`playable_session.*` 編排三層駐留，`session_persistence.*` 編排世界存讀，`save_raws.*` 管存檔基底 TOML 的不可變複製／雜湊／路徑 |
...
| `runtime/` | 三層進退、上層回寫、親自／代管校準、session 磁碟冷存冷讀／pending 拒存，以及世界槽自帶 raws 的隔離／竄改拒絕 |
