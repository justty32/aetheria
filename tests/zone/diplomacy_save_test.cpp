...
TEST(DiplomacySave, V14RootLoadsWithDiplomacyAbsentRatherThanNeutral) {
    const auto loaded = aetheria::serialize::decode_zone(encode_v14_root_zone(test_ruleset()),
                                                         test_ruleset());
    ASSERT_NE(loaded, nullptr);
...
TEST(DiplomacySave, V15FixtureKeepsAllV17FieldsAbsent) {
    ...
    std::cout << "diplomacy_v15 named_fate=0 truth_presence=0 knowledge_presence=0"
                 " mind_presence=0\n";
}

TEST(DiplomacySave, V17StoreRejectsV15Manifest) {
    TemporaryDirectory directory;
    ...
    try {
        ...
        FAIL() << "v15 manifest should be rejected by v17 store";
    } catch (const std::runtime_error& error) {
        std::cout << "diplomacy_v15_reject_error=" << error.what() << '\n';
        EXPECT_NE(std::string{error.what()}.find("檔內=15 預期=17"), std::string::npos);
    }
}
