...
claim_all_land(const world::RegionTiles& tiles, std::span<const InfluenceCapital> capitals,
    ...
    if (!tiles.valid_layout() ||
        ...
        throw std::invalid_argument{"全域認領需要有效且可用 RegionXY 表示的 RegionTiles"};
    }
    if (influence_season < 1 || influence_season > 4) {
        throw std::invalid_argument{"全域認領季節無效"};
    }
        ...
        for (const auto& [dx, dy] : kDirections) {
            ...
            const auto step = influence_terrain_step_cost(
                ruleset, {tiles.base[next_index], tiles.relief[next_index],
                          tiles.feature[next_index], influence_season});
            if (current.key.cost > std::numeric_limits<std::int64_t>::max() - step) {
                throw std::overflow_error{"全域認領成本超出 int64"};
            }
    ...
    InfluenceClaimMap claims;
    claims.owner.assign(count, world::FactionId{0});
    claims.capital_cost.reserve(count);
    for (std::size_t index = 0; index < count; ++index) {
        claims.owner[index] = best[index].cost == std::numeric_limits<std::int64_t>::max()
                                  ? world::FactionId{0}
                                  : best[index].faction;
        claims.capital_cost.push_back(best[index].cost);
    }
    measured.maximum_updates_per_tile =
        updates.empty() ? 0U : *std::max_element(updates.begin(), updates.end());
