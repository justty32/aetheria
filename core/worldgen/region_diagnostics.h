...
// region_diagnostics.h 收斂十二階段雜湊、骨架／tile 雜湊、陸地連通檢查與灰階視覺化宣告。
...
[[nodiscard]] std::uint64_t hash_stage(const PortalStageOutput& stage) noexcept;
[[nodiscard]] std::uint64_t hash_stage(const FactionStageOutput& stage) noexcept;
...
[[nodiscard]] std::vector<std::uint8_t> grayscale(const PortalStageOutput& stage);
[[nodiscard]] std::vector<std::uint8_t> grayscale(const FactionStageOutput& stage);
