...
3. 以整數亮度梯度判斷四象限主光。`terrain` 旋轉至左上主光；`object`（含角色類）只輸出
   偵測方向，絕不旋轉。無有效梯度時回報 `undetermined`。
...
光源判定、旋轉角、光源警告、修補前後接縫度量與 SHA-256。非左上主光的 `object` 預設只警告，
加 `--object-light-policy ignore` 可關閉警告；兩種模式都不會因此拒絕處理。
...
{
  "assets": [
    {"def_id": "terrain.grassland", "size": [64, 64], "kind": "terrain"},
    {"def_id": "actor.guard", "size": [64, 96], "kind": "object"}
  ]
}
```
...
manifest 將素材標成 `kind: object` 時，入庫閘會偵測主光；非左上只印警告並維持退出碼 0。
`--object-light-policy ignore` 可關閉這項警告，目前沒有 `fail` 模式。

`verify_acceptance.py` 不只跑正向案例，也會在暫存副本自行注入無序色盤、跳過 terrain 旋轉、
跳過接縫修補與逐一停用四種入庫規則。任何故障控制沒有讓對應數字改變，腳本都會退出 1。

## 已知界線

- 去背是依既有 alpha 或指定背景色的確定性規則，不是語意分割。
- 梯度只能判定明暗主軸；多重光源或無方向性光線會回報 `undetermined`。
- `object` 的錯向主光目前只警告；是否退回重生成仍由接觸表與人工檢視決定。
