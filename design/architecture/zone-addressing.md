...
> [本層入口](README.md)｜從 [zone-model.md](zone-model.md) 拆出。那份講 zone 的**結構與生命週期**，
...
| **投影天然可定址** | [interface-world-mid.md](../simulation/interface-world-mid.md) 的 `project(region_tile)` 需要「這格的 Site 是誰」——一次位元運算 |
| **接邊的規範化 id 直接可算** | [edge-consistency.md](../generation/edge-consistency.md) 的 `canonical_edge_id` 需要兩側的格位，從 key 就有 |
...
這正好對應 [interface-lifecycle.md](../simulation/interface-lifecycle.md) 的 `L_ABSENT`：
位址永遠算得出來，內容按需生成或載入。
