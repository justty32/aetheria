...
> [本層入口](README.md)｜**重要發現（2026-08-15）：`~/repo/game_dev/medps` 是同一個構想的前一輪，而且走得相當遠。**
...
|---|---|---|---|
| 1 | 方格 4 鄰接 | [worldmap.md](../maps/worldmap.md) | ✅ 繼承 |
| 2 | def 進不可變 Ruleset、不進存檔、id→下標索引 | [definitions.md](../rules/definitions.md) | ✅ 繼承 |
| 3 | 垂直層收在同一 zone 的 z 鍵 | [outline.md](../outline.md) | ✅ 繼承 |
| 4 | 成長軸不變量 | [observer.md](../simulation/observer.md) | ✅ 繼承 |
| 5 | 逐出只在 tick 尾端 + `pinned` 第二道防線 | [interface-lifecycle.md](../simulation/interface-lifecycle.md)、[event-scaling.md](../simulation/event-scaling.md) | ✅ 繼承（LRU 換成場強） |
| 6 | 跨 zone 引用用 zone id + 穩定 uid，解引用可失敗 | [events.md](../simulation/events.md) 的 `participants` | ✅ 繼承 |
