...
> [本層入口](README.md)｜zone 怎麼落到磁碟、怎麼讀回來、格式變動怎麼辦。
...
`world_seed` **不足以**重現世界。程序層（地形骨架）依
[zone-save.md 的「三層資料只存中間那層」](#三層資料只存中間那層)**不進存檔、每次重算**——
所以重算時用的**生成參數**（板塊數、octave 數、侵蝕次數、talus、海平面二分次數、
目標陸地比…）與 `world_seed` 一樣是輸入。

參數不固定的後果是安靜的：某次改了預設侵蝕次數，**既有存檔下次載入時地形就變了**，
玩家蓋在山谷裡的工坊發現自己站在山脊上。沒有任何錯誤訊息。

**規則**：manifest 存**生成參數的雜湊**（或整組參數本身），載入時比對，
不符即 fail-fast 並指出是哪一組。`world_seed` 與參數雜湊**合起來**才是世界的身分。

這與 `WorldDims` 宣告 IMMUTABLE 是同一個理由，只是它更容易被忽略——
...
（見 [event-scaling.md](../simulation/event-scaling.md)）。
