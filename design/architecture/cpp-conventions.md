...
> [本層入口](README.md)｜承接 [tech-stack.md](tech-stack.md) 的三層架構，這份講「core 用什麼寫、怎麼寫、靠什麼建置」。
...
- **「種類」一律不是 enum，是資料檔定義的下標。** `TerrainId`、`EdgeId`、`BuildingDefId`…
  ...
  完整規則見 [definitions.md](../rules/definitions.md)。
...
- **時間分兩種**：`Tick`（時刻）與 `Duration`（時距），都是 `enum class X : int64_t {}` 的秒，
  ...
  **[time-model.md](../simulation/time-model.md)**。
...
- 格子資料用 **SoA**（見 [worldmap.md](../maps/worldmap.md)）。
...
| 用途 | 套件 | 備註 |
|---|---|---|
| 腳本 | `lua`、`sol2` | 規則擴展層，見 [rules-extensibility.md](../rules/rules-extensibility.md) |
...
詳見 [rules-extensibility.md](../rules/rules-extensibility.md)。
