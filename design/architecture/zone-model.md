...
> [本層入口](README.md)｜**M0.5 的直接前置。** 三層地圖在程式裡是同一種東西：zone。
...
「種類一律是資料」那條（[definitions.md](../rules/definitions.md)）：那條管的是**種類**，這是 **schema**。
...
| 全局時鐘、曆法、季節 |
| 劇情旗標 |
| **觀察點樹**（見 [observer.md](../simulation/observer.md)） |
| **獨特物件登記表**（見 [unique-objects.md](../simulation/unique-objects.md)） |
...
| `materialize(key)` | 不存在就生成（走 [gen-pipeline.md](../generation/gen-pipeline.md)） |
...
所以 [observer.md](../simulation/observer.md) 那條繼承自 medps 的不變量在這裡是硬性的：
...
- `SiteTiles`／`LocalTiles` 的欄位集（L1 的 `RegionTiles` 已定，見 [worldmap.md](../maps/worldmap.md)；
