...
> [本層入口](README.md)｜承接 [outline.md](../outline.md)（全局常數：秒、stride 表、360 天曆）與
> [cpp-conventions.md](../architecture/cpp-conventions.md)（型別約定）。
...
**不准出現裸 `int` 的「回合數」**——理由見 [cpp-conventions.md](../architecture/cpp-conventions.md)：
...
不是「預期內的失敗」——依 [cpp-conventions.md](../architecture/cpp-conventions.md) 的錯誤處理約定，
...
[cpp-conventions.md](../architecture/cpp-conventions.md) 早就要求「不變式用 `assert`，
...
**獨立一檔** → [time-clock-authority.md](time-clock-authority.md)。
「旬內小時不是 Site 的狀態」那條裁定，以及時距／時戳／時鐘讀數的三分在那裡。
