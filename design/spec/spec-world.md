...
下一層導引：[整合推演](spec-integration.md)、[裁定清單](spec-decisions.md)、[驗證矩陣](spec-verification.md)、[交接與後續](spec-review-next.md)。數值不確定不影響讀懂規則；會改行為的選擇必須先裁定再實作。
...
本輪不包含：程式與 API、存檔格式升版、全量內容及平衡數值、素材製作、實機校準、實作任務書、commit／push。mark／獨特物件追蹤與 root 成長仍擱置；一般穩定身分與故事歷史不因此停擺。
