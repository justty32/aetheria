# 三層世界運作與共用規格

← [上層入口](../README.md)

先讀整體運作，再讀大地圖與地方／人物如何接手。S0–S1 負責身分、責任交接與因果時序，不另外造一份世界。

本 README 兼本層索引，只列本資料夾文件；跨領域依賴在各篇正文連結。

| 文件 | 職責 |
|---|---|
| [spec-world-operation.md](spec-world-operation.md) | 無人類命令時的完整循環、NPC 情況分類與日後接入使用者 |
| [spec-world-region-flow.md](spec-world-region-flow.md) | 大地圖環境、組織目標、跨區安排與接收後果 |
| [spec-world-place-flow.md](spec-world-place-flow.md) | 中層調度、小層人物行動、平戰與中斷接續 |
| [spec-world-cases.md](spec-world-cases.md) | 七條純 NPC 情境：日常與變故、追加內容、長期等待 |
| [spec-world-border.md](spec-world-border.md) | 邊境商路例子：勢力、軍隊、城區與人物選擇如何互相改變 |
| [spec-world-growth-decline.md](spec-world-growth-decline.md) | 和平投資、移居與學藝，以及衰退、撤軍、協商和重組 |
| [spec-world-eras.md](spec-world-eras.md) | 大災難、各方應對與長期格局；紀元稱呼不重置世界 |
| [spec-world-adventure.md](spec-world-adventure.md) | 冒險隊與魔王部下自行行動，串起古塔危機與村鎮後續 |
| [spec-world-magic.md](spec-world-magic.md) | 奇幻手段如何改變各方選擇、依賴與回應 |
| [spec-world-handoff.md](spec-world-handoff.md) | S0：身分、責任範圍與交接 |
| [spec-world-time.md](spec-world-time.md) | S1：時間、效果提交與中斷 |
