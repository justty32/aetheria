...
> [本層入口](README.md)｜**鐵律：地形、河流、道路、建築、單位這類「種類」一律不寫死成 C++ enum。**
> 它們是資料檔裡的定義物件，執行期以整數下標存取。
> 上層見 [tech-stack.md](../architecture/tech-stack.md)；擴展層見 [rules-extensibility.md](rules-extensibility.md)。
...
| `BuildingDef` | 建築 |
| `UnitDef` | 單位 |
| `ZoneKindDef` | Site 種類（城市／要塞／荒野…，見 [midmap.md](../maps/midmap.md)） |
...
但 [lowmap.md](../maps/lowmap.md) 的持久層表已經寫著**血跡「短期存，超時清理」**。
...
[worldmap.md](../maps/worldmap.md) 的 SoA 欄位存的是 **def 下標，不是 enum 值**：
