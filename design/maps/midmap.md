...
> [本層入口](README.md)｜Region 的一格放大後的樣子。一小時一回合。玩法是城建經營或 SRPG 戰略。
> 上層見 [outline.md](../outline.md)；與大地圖的界面見 [interface-world-mid.md](../simulation/interface-world-mid.md)。
...
**牆是邊屬性**，和 Region 的河流道路共用同一套 `EdgeDef` 機制（見 [definitions.md](../rules/definitions.md)）：
...
哪些 Site 活著、用什麼解析度活著，由**觀察點機制**決定（[observer.md](../simulation/observer.md)）：
...
至於 Site 裡的**哪些實體**要個別計算，是另一條軸——見 [significance.md](../simulation/significance.md)。

完整的具現化／卸載／補算規則，以及它為什麼不會讓世界狀態漂移，
在 [interface-world-mid.md](../simulation/interface-world-mid.md) 與 [interface-lifecycle.md](../simulation/interface-lifecycle.md)。

## 生成流程

| Site 種類 | 文件 |
|---|---|
| `City` / `Town` / `Fortress` | [sitegen-city.md](../generation/sitegen-city.md) — 骨架（主幹道→街廓）先行，填充（分區→建築→城牆）在後 |
| `Wilderness` / `Ruin` / `Sea` | [sitegen-wild.md](../generation/sitegen-wild.md) — 更便宜、持久層極小，是 LOD 機制最常運作的地方 |

**接邊一致性**是這兩者共同的前提：相鄰 Site 的河流、道路、海岸線必須嚴絲合縫。
作法是**邊界由更低維的物件裁決**（角 → 邊 → 面），Site 永遠不寫自己的邊界，
邊界是它的輸入。完整機制與四項驗證測試見 [edge-consistency.md](../generation/edge-consistency.md)。

生成預算 **< 30 毫秒**（玩家進城的當下生成，不能卡），
因此禁止任何迭代式或回溯式演算法——見 [gen-pipeline.md](../generation/gen-pipeline.md)。

## 待細化

- 建築表、分區規則、相鄰加成的完整數值
- SRPG 的行動點與陣型規則（戰鬥的期望值基準已定，見 [combat-formula.md](../rules/combat-formula.md)）
