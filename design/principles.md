...
> **第一個原型之後可能全部打掉重做。到時候要能把組件一個個拆走再利用——
> 耦合不深、介面窄、各自成立。**

推論出的具體規矩：

- **新能力＝新模組**，不長在既有的 god object 上（`PlayableSession` 是頭號嫌犯——
  它已經揹著場景、校準、UI 佇列；再往裡塞就是在給未來的重做挖坑）。
- **依賴單向**：通用組件（存檔、日誌、規則庫）不准知道使用它的 session／bridge。
- 一個組件「拆得走」的判準：**把它連同測試複製到新 repo，補上它宣告的少數依賴就能編**。
