...
> [本層入口](README.md)｜相鄰的兩個 Site 各自獨立生成，它們的共用邊界必須嚴絲合縫：
...
牆與河流道路共用同一套 `EdgeDef`（見 [definitions.md](../rules/definitions.md)），
所以這一欄用的是同一個型別。
...
| **出境點（portal tile）** | [worldmap.md](../maps/worldmap.md) 的 WorldGraph 邊。相連的兩個 Region **在空間上不相鄰**，所以**不要求幾何連續**——只要求敘事連續（碼頭對碼頭、山口對山口） |
