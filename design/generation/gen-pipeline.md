...
> [本層入口](README.md)｜三層地圖的生成器共用同一套骨架與同一套規矩。這份講規矩，
...
呼應 [interface-world-mid.md](../simulation/interface-world-mid.md) 的骨架穩定性鐵律：
...
aetheria 一開始就不走那條路。詳見 [definitions.md](../rules/definitions.md)。
...
> **裁定：荒野 Site 另設更嚴的回歸預算 `< 5 ms`（Release，2026-08-21）**
>
> 30 ms 是**單一 Site 的絕對上限**，對城區合適（玩家進城是一次性動作）。
> 但荒野 Site 會**被大量生成**——玩家在 Region 上移動時每經過一格就是一個。
> 一格 30 ms 在串流時會疊成災難，所以荒野另立 `< 5 ms` 的回歸預算。
> 實測目前 Release 最差 `0.674 ms`，餘裕七倍。
>
> ⚠ **批次／預取的總預算現在不定。** 串流路徑要到 M5 才存在，
> 沒有東西可以拿來判斷「一次預取幾個」才合理——**現在定就是憑空猜**。
...
`aetheria_sim`（headless CLI，見 [cpp-conventions.md](../architecture/cpp-conventions.md)）應提供：
